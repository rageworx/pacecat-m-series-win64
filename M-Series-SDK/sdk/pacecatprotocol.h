#ifndef __PACECATPROTOCOL_H__
#define __PACECATPROTOCOL_H__
...
// debug   factor _____________________________________________________________
// ZERO_POINT_FACTOR:     Functional sound ratio
// DISTANCE_CLOSE_FACTOR: Within 0.2m
// SUM_POINT_NUM_FACTOR:  Total points percentage
#define ZERO_POINT_FACTOR       0.8
#define DISTANCE_CLOSE_FACTOR   0.8
#define SUM_POINT_NUM_FACTOR    0.2

// POINTCLOUD_TIMESTAMP_MAX_DIFF:     Maximum time interval of point cloud 
//                                    packets
// IMU_TIMESTAMP_MAX_DIFF:            Maximum time interval of IMU packets
// POINTCLOUD_IMU_TIMESTAMP_MAX_DIFF: Maximum time interval between point cloud 
//                                    packets and IMU packets
// HEART_INTERVAL:                    Heart rate monitoring interval
#define POINTCLOUD_TIMESTAMP_MAX_DIFF           25000000
#define IMU_TIMESTAMP_MAX_DIFF                  7500000
#define POINTCLOUD_IMU_TIMESTAMP_MAX_DIFF       25000000
#define LOG_TIMER                               2
...
// Heartbeat IMU data packets
struct IMU_DATA_PACK_D4 
{
    uint8_t     D4;             /// == 0xD4
    uint8_t     AccelScale : 4; /// Accelerate measurement range, value SET_ACCEL_FS_SEL_*
    uint8_t     GyroScale : 4;  /// Angular velocity range, value SET_GYRO_FS_SEL_*
    int16_t     Accel_X;        /// Actual value = Accel_X * Accel range / 32768.0
    int16_t     Accel_Y;
    int16_t     Accel_Z;
    int16_t     Gyro_X;         /// Actual value = Gyro_X * angular velocity range / 32768.0
    int16_t     Gyro_Y;
    int16_t     Gyro_Z;
    int16_t     Temp;
    int8_t      t;
...
{
    int     sfp_enable;
    int     window;         /// Shadow detection window size
...
{
    int     dfp_enable;
    int     continuous_times;   /// Sustained frame rate
    double  dirty_factor;       /// Dirt and stain alarm coefficient
...
{
    int     pfp_enable;
    int     window_num;       /// Shadow detection window size
    double  effective;
    double  threshold;
} PointFilterParam;
...
struct DeBugInfo
{
    // The last timestamp with an excessively large time interval
    uint64_t pointcloud_timestamp_large_last;
    // Print count statistics for the current time period
    uint32_t pointcloud_timestamp_large_num;

    // The timestamp of the last time jump
    uint64_t pointcloud_timestamp_jumpback_last;
    // Print count statistics for the current time period
    uint32_t pointcloud_timestamp_jumpback_num;
    
    // The timestamp of the last packet loss
    uint64_t pointcloud_timestamp_drop_last;
    // Print count statistics for the current time period
    uint32_t pointcloud_timestamp_drop_num;

    // Radar last update timestamp
    uint64_t pointcloud_timestamp_last;
    // System last update timestamp
    uint64_t system_timestamp_last; 
    // Does point cloud packet exist over a period of time?
    uint8_t  pointcloud_exist;
    // Does the IMU package exist within a certain period of time?
    uint8_t  imu_exist;

    // Point cloud package subscript
    int16_t  pointcloud_packet_idx;

    // The last timestamp with an excessively large time interval
    uint64_t imu_timestamp_large_last;
    // Print count statistics for the current time period
    uint32_t imu_timestamp_large_num;

    // The timestamp of the last time jump
    uint64_t imu_timestamp_jumpback_last;
    // Print count statistics for the current time period
    uint32_t imu_timestamp_jumpback_num;

    // The timestamp of the last lost IMU packet
    uint64_t imu_timestamp_drop_last;
    // Print count statistics for the current time period
    uint32_t imu_timestamp_drop_num;

    // Radar last update timestamp
    uint64_t imu_timestamp_last;
    // IMU package index
    int16_t  imu_packet_idx;

    // The timed detection period is tentatively set at 1 second.
    uint64_t  timer;

    // Point cloud timestamp interval too large alarm
    uint64_t pointcloud_imu_timestamp_large_last;
    // Print count statistics for the current time period
    uint32_t pointcloud_imu_timestamp_large_num;

    // Mirror rotation anomaly timestamp
    uint64_t mirror_err_timestamp_last;
    // Print count statistics for the current time period
    uint32_t mirror_err_timestamp_num;

    // Base plate abnormal timestamp
    uint64_t motor_err_timestamp_last;
    // Print count statistics for the current time period
    uint32_t motor_err_timestamp_num;

    // Dirty data abnormal timestamp
    uint64_t dirtydata_err_timestamp_last;
    uint64_t dirtydata_err_timestamp_num;

    // Remove abnormal timestamps for dragging points
    uint64_t filterdata_err_timestamp_last;
    uint64_t filterdata_err_timestamp_num;

    // Zero point timestamps.
    uint64_t zero_pointdata_timestamp_last;
    uint64_t zero_pointdata_timestamp_num;

    // Distance Too Close Coefficient Alarm Timestamp
    uint64_t distance_close_timestamp_last;
...
#endif /// of __PACECATPROTOCOL_H__