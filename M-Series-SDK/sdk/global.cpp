...
bool BaseAPI::checkAndMerge(int type, char *ip, char *mask, char *gateway, int port, char *result)
{
    ...
    if (type == 0)
    {
        // dangerous casting without buffer length !
        if ( result != nullptr )
            sprintf(result, "%03d.%03d.%03d.%03d %05d", 
...
{
#ifdef _WIN32
    // Is this a way to init WSA ? 
    // must be moved to somewhere platform dep. part to do single startup.
...
std::string SystemAPI::getCurrentTime()
{
    ...
    snprintf(result, 80, "%04d%02d%02d_%02d%02d%02d.%03lld", 
