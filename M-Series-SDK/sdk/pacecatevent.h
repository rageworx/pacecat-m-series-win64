...
//CMD: LEVENTRH  Read All Event Log. 
//     The command to read radar events returns a SYS_EVENT_LOG structure. 
//     The software needs to parse the event type of the data packet.
//CMD: LEVENTCH  Clear ALL Event Log 
//     Clear radar event command,
//     but will log one clear event (EVENT_ID_EVENT_CLEARED).
...
#define EVENT_ID_NET_STACK_ERR 4        /// A network protocol stack error can cause the system to reset.
...
typedef  struct 
{
    uint32_t  Header;                           /// Magic4 == 0xEFDC55AA
