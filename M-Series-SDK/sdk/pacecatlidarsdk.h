 ...
 *    - Raphael.Kim , 2026-02-02, 2026-02-19
 * -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
 *
 * Description: 
...
struct RunConfig
{
    ...
    uint16_t                            package_num_idx{0}; /// Current frame count
    int                                 timemode;
    uint8_t                             rain;
    uint8_t                             echo_mode;
    std::string                         log_path;
...
class PaceCatLidarSDK
{
    ...
    private:
        //Check if the printing exception is caused by a callback blocking issue.
